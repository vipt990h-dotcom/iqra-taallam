...
async function startRecording(){
  ...
  try{
    ...
    mediaRecorder.onstop=()=>{
      currentAudioBlob=new Blob(chunks,{type:'audio/webm'});
      document.getElementById('audioPreview').src=URL.createObjectURL(currentAudioBlob);
      s.innerHTML='تم حفظ التسجيل مؤقتًا. استمعي له قبل الإرسال.';
    };
...
function saveEvaluation(){
  ...
  if(msg) msg.innerHTML='تم حفظ تقييم المعلمة تجريبيًا وسيظهر لولي الأمر عند ربط قاعدة البيانات.';
}
...
function exportReport(name){
  ...
  const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='iqra-report-'+name+'.txt'; a.click();
}
